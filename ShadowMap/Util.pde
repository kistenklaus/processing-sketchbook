static class Util{
  ...
  public static PVector lineIntersectionPoint(PVector A, PVector B, PVector C, PVector D){
    if(!lineIntersect(A,B,C,D))return null;
    float a1 = B.y - A.y;
    float b1 = A.x - B.x;
  ...
  public static boolean insideOfQuad(PVector center, PVector dim, PVector point, float epsilon){
    PVector pos = new PVector(center.x - dim.x/2.0f, center.y - dim.y/2.0f);
    
    return (
           pos.x <= point.x + epsilon 
        && pos.y <= point.y + epsilon
        && pos.x + dim.x + epsilon >= point.x
        && pos.y + dim.y + epsilon >= point.y);
